...
import cn.rongcloud.chatroomkit.R.string;
...
public class PermissionCheckUtil {
        ...
        if (permissions.length == 0) {
            ...
            for (int var7 = 0; var7 < var6; ++var7) {
                ...
                if ((isFlyme() || VERSION.SDK_INT < 23) && permission.equals("android.permission.RECORD_AUDIO")) {
                    ...
                    if (isPrompt) {
                        showPermissionAlert(fragment.getContext(), fragment.getString(string.rckit_permission_grant_needed) + fragment.getString(string.rckit_permission_microphone), new OnClickListener() {
    ...
    public static boolean checkPermissions(Context context, @NonNull String[] permissions) {
        if (permissions.length == 0) {
            ...
            for (int var4 = 0; var4 < var3; ++var4) {
                ...
                if ((isFlyme() || VERSION.SDK_INT < 23) && permission.equals("android.permission.RECORD_AUDIO")) {
                    VMLog.d(TAG, "Build.MODEL = " + Build.MODEL);
    ...
    private static boolean hasRecordPermision(Context context) {
        ...
        if (bufferSizeInBytes < 0) {
            VMLog.e(TAG, "bufferSizeInBytes = " + bufferSizeInBytes);
            return false;
        } else {
            try {
                ...
                VMLog.e(TAG, "Audio record exception.");
            }
    ...
    private static String getNotGrantedPermissionMsg(Context context, String[] permissions, int[] grantResults) {
        if (permissions != null && permissions.length != 0) {
            try {
                ...
                StringBuilder builder = new StringBuilder(context.getResources().getString(string.rckit_permission_grant_needed));
                return builder.append("(").append(TextUtils.join(" ", permissionNameList)).append(")").toString();
            } catch (NotFoundException var6) {
                VMLog.e(TAG, "One of the permissions is not recognized by SDK." + Arrays.toString(permissions));
                return "";
            }
        } else {
            return "";
    ...
    private static String getNotGrantedPermissionMsg(Context context, List<String> permissions) {
        if (permissions != null && permissions.size() != 0) {
            ...
            try {
                ...
                VMLog.e(TAG, "one of the permissions is not recognized by SDK." + permissions.toString());
                return "";
            }
    ...
    private static void showPermissionAlert(Context context, String content, OnClickListener listener) {
        (new Builder(context)).setMessage(content).setPositiveButton(string.rckit_confirm, listener).setNegativeButton(string.rckit_cancel, listener).setNeutralButton(string.rckit_not_prompt, listener).setCancelable(false).create().show();
    }
        ...
        if (VERSION.SDK_INT >= 23) {
            try {
                ...
                if (!booleanValue && needOpenPermissionSetting) {
                    ...
                    showPermissionAlert(context, context.getString(string.rckit_permission_grant_needed) + getNotGrantedPermissionMsg(context, permissionList), new OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            if (-1 == which) {
                ...
                VMLog.d(TAG, "isFloatWindowOpAllowed allowed: " + booleanValue);
                return booleanValue;
            } catch (Exception var7) {
                VMLog.e(TAG, String.format("getDeclaredMethod:canDrawOverlays! Error:%s, etype:%s", var7.getMessage(), var7.getClass().getCanonicalName()));
                return true;
            }
            ...
            try {
                ...
                VMLog.e(TAG, String.format("NoSuchMethodException method:checkOp! Error:%s", var8.getMessage()));
                method = null;
            } catch (ClassNotFoundException var9) {
                VMLog.e(TAG, "canDrawOverlays " + var9);
                method = null;
            }
            ...
            if (method != null) {
                try {
                    ...
                    VMLog.e(TAG, String.format("call checkOp failed: %s etype:%s", var10.getMessage(), var10.getClass().getCanonicalName()));
                }
            }

            VMLog.d(TAG, "isFloatWindowOpAllowed allowed: " + result);
            return result;
        }
    }
    ...
    private static boolean hasPermission(Context context, String permission) {
        ...
        if (opStr == null && VERSION.SDK_INT < 23) {
            ...
            return context != null && context.checkCallingOrSelfPermission(permission) == PERMISSION_GRANTED;
        }
    }
    ...
    public static void showRequestPermissionFailedAlter(final Context context, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (!TextUtils.isEmpty(content)) {
            ...
            if (VERSION.SDK_INT >= 21) {
                (new Builder(context, 16974394)).setMessage(content).setPositiveButton(string.rckit_confirm, listener).setNegativeButton(string.rckit_cancel, listener).setCancelable(false).create().show();
            } else {
                (new Builder(context)).setMessage(content).setPositiveButton(string.rckit_confirm, listener).setNegativeButton(string.rckit_cancel, listener).setCancelable(false).create().show();
            }
